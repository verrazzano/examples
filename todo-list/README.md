...
4. [Initialize the database](#initialized-the-database).
...
need this installer again to build the Docker image.  To make copying the commands a little easier,
...
Using the Oracle WebLogic Server Configuration Wizard, create a new domain called `tododomain`.  Add the password for the admin
...
required to match the application and database settings from when you started and created the MySQL database:
    ...
    - Navigate to Services->Data Sources in the WebLogic Server Administration Console
...
Unzip the installer `weblogic-deploy.zip` so that you can access `bin/discoverDomain.sh`. To make copying the
...
- **wdt-archive.zip** - The WDT archive containing the ToDo List application WAR
...
verify that you see the `todo.war` file inside the `wdt-archive.zip` file.  If you do not see the WAR, there was something wrong
...
Unzip the installer `imagetool.zip` so that you can access `bin/imagetool.sh`. To make copying the commands a
...
# Paths for the files in this command assume that you are running it from the v8o directory created during the `discoverDomain` step above.
